...
const inputPath = path.join(root, 'pml-coding-assistant/production-reference/DBListing-examples/DBOutput_PIPING.txt');
const reportsRoot = path.join(root, 'user-support/kpi-reports');
...
function makeReportManifest(report, data) {
  return {
    ...
    generator: 'user-support/kpi-reports/scripts/generate_demo_kpi_reports.js',
...
function baseModel(records, sourceText) {
  ...
  return {
    ...
    source: {
      ...
      path: 'pml-coding-assistant/production-reference/DBListing-examples/DBOutput_PIPING.txt',
...
function writeIndex() {
  const rows = fs.readdirSync(reportsRoot)
    .map((entry) => path.join(reportsRoot, entry))
    .filter((entryPath) => fs.existsSync(path.join(entryPath, 'report.json')))
    .map((entryPath) => {
      const meta = JSON.parse(fs.readFileSync(path.join(entryPath, 'report.json'), 'utf8'));
      return {
        title: meta.title,
        slug: meta.slug,
        status: meta.status,
